...
class PatientDocumentsRepository:
    ...
    def get_patient_folders(self, patient_id: int) -> List[Dict]:
        ...
        with SessionLocal() as session:
            ...
            """)
            folders = session.execute(folders_query, {"patient_id": patient_id}).fetchall()
            ...
            for folder in folders:
                ...
                doc_count_query = text("""
                    SELECT COUNT(*) 
                    FROM patient_documents 
                    WHERE patient_id = :patient_id AND folder_id = :folder_id AND is_archived = FALSE
                """)
                doc_count = session.execute(doc_count_query, {"patient_id": patient_id, "folder_id": folder.id}).scalar()
                ...
                last_doc_query = text("""
                    ...
                    LIMIT 1
                """)
                last_doc = session.execute(last_doc_query, {"patient_id": patient_id, "folder_id": folder.id}).scalar()
    ...
    def create_folder(self, patient_id: int, name: str, created_by: int) -> Dict:
        ...
        with SessionLocal() as session:
            folder = PatientDocumentFolderORM(
    ...
    def archive_document(self, document_id: int) -> bool:
        ...
        with SessionLocal() as session:
            document = session.query(PatientDocumentORM).filter(
                PatientDocumentORM.id == document_id
            ).first()
